...
def getArgs():
  ...
  parser = argparse.ArgumentParser(
      description="Power VMs off if they are on",
...
if __name__ == "__main__":
  ...
  for vm in filter(
    ...
    #We need to power off the VM only if it is powered on:
    if vm.runtime.powerState == vim.VirtualMachine.PowerState.poweredOn:
      task = vm.PowerOffVM_Task()
      WaitForTask(task, si=si, onProgressUpdate=generateProgress('Powering off VM ' + vm.name))
  Disconnect(si)
