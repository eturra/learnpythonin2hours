...
def getArgs():
  ...
  parser.add_argument('-G', '--guestos', required=False, action='store_true', help='Print also the guestOS of the VM')
  parser.add_argument('-E', '--esxhost', required=False, action='store_true', help='Print also the ESXi host running the VM')
  parser.add_argument('-A', '--annotation', required=False, action='store_true', help='Print also the annotation of the VM')
  parser.add_argument('-R', '--reservations', required=False, action='store_true', help='Print also the cpu and memory reservations of the VM')
...
if __name__ == "__main__":
  ...
  for vm in filter(
    ...
    if args.guestos:
      print(' ', vm.guest.guestFullName, end='')
    if args.esxhost:
      print(' ', vm.runtime.host.name, end='')
    if args.annotation:
      print(' ', vm.config.annotation, end='')
    if args.reservations:
      ...
      print(' CPU:', vm.config.cpuAllocation.reservation, 'out of', cpus.get(vm.runtime.host.name), 'Memory:', vm.config.memoryAllocation.reservation, 'out of', vm.config.hardware.memoryMB, end='')
    print()
  Disconnect(si)
