...
if __name__ == "__main__":
  ...
  for vm in filter(
    ...
    cs = vim.vm.ConfigSpec(
        ...
        cpuAllocation = vim.ResourceAllocationInfo(reservation=0),
        memoryAllocation = vim.ResourceAllocationInfo(reservation=0),
        )
    task = vm.ReconfigVM_Task(spec=cs)
    WaitForTask(task, si=si, onProgressUpdate=generateProgress('Removing any memory or cpu reservations from VM ' + vm.name))
  Disconnect(si)
